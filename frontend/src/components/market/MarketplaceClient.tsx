        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-6">
                <div className="text-center space-y-4">
                    ...
                    <button onClick={() => window.location.reload()} className="px-6 py-2 bg-black/[0.03] border border-black/[0.06] rounded-xl hover:bg-black/[0.05] transition-all">
                        Retry
                    </button>
                </div>
            </div>
        ...
        <div className="min-h-screen p-6 lg:p-12 space-y-12 max-w-7xl mx-auto pb-24">
            ...
            <header className="relative space-y-6 max-w-3xl">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest text-primary">Global Protection Market</span>
                </div>
                    ...
                    <div className="flex items-center justify-between px-6 py-4 border-b border-black/[0.04]">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-[#71717A] uppercase tracking-widest font-bold">4-Step Guide to Parametric Protection</p>
                            </div>
                        </div>
            ...
            {/* Controls */}
            <section className="sticky top-20 z-30 py-4 bg-background/80 backdrop-blur-xl border-y border-black/[0.04] -mx-6 px-6">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative flex-1 group">
                        ...
                        <input
                            ...
                            className="w-full bg-zinc-900/50 border border-black/[0.04] rounded-2xl pl-12 pr-4 py-4 text-sm focus:outline-none focus:border-primary/50 transition-all"
                        />
                    </div>
                    <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                        <div className="flex bg-zinc-900/50 border border-black/[0.04] rounded-2xl p-1 gap-1">
                            {categories.slice(0, 4).map((cat) => (
                                <button
                                    key={cat.id}
                                    onClick={() => setSelectedCategory(cat.id)}
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap
                                        ${selectedCategory === cat.id ? 'bg-primary text-[#1A1A1A] shadow-lg' : 'text-slate-500 hover:text-slate-300 hover:bg-black/[0.03]'}`}
                                >
                        ...
                        <button className="p-3 bg-zinc-900/50 border border-black/[0.04] rounded-2xl text-slate-500 hover:text-[#1A1A1A] transition-all">
            ...
            <section className="space-y-8">
                <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-black/[0.03] px-3 py-1 rounded-full">
                ...
                {isLoading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {Array.from({ length: 6 }).map((_, i) => (
                            <div key={i} className="h-[420px] bg-zinc-900/50 border border-black/[0.04] rounded-3xl animate-pulse" />
                        ))}
                    </div>
                ) : filteredProducts.length === 0 ? (
                    <div className="py-24 text-center border border-dashed border-black/[0.06] rounded-3xl">
            ...
            {/* Trust Footer */}
            <section className="py-12 border-t border-black/[0.04] grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left">
                <div className="space-y-2">
                    <div className="flex items-center gap-2 justify-center md:justify-start text-primary font-bold uppercase text-[10px] tracking-widest">
