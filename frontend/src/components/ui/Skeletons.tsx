        ...
        <div className="glass-panel rounded-xl p-6 space-y-4 animate-pulse">
            <div className="flex items-center gap-3">
                ...
                <div className="space-y-2 flex-1">
                    ...
                    <div className="h-3 bg-black/[0.03] rounded w-1/3 skeleton-shimmer" />
                </div>
            </div>
            <div className="space-y-2">
                <div className="h-3 bg-black/[0.03] rounded w-full skeleton-shimmer" />
                <div className="h-3 bg-black/[0.03] rounded w-4/5 skeleton-shimmer" />
            </div>
        </div>
    );
}
...
export function TableSkeleton({ rows = 3 }: { rows?: number }) {
    return (
        <div className="space-y-3 animate-pulse">
            <div className="flex gap-4 px-4 py-3 border-b border-black/[0.04]">
                {[1, 2, 3, 4].map((i) => (
            ...
            {Array.from({ length: rows }).map((_, i) => (
                <div key={i} className="flex gap-4 px-4 py-3">
                    {[1, 2, 3, 4].map((j) => (
                        <div key={j} className="h-3 bg-black/[0.03] rounded flex-1 skeleton-shimmer" />
                    ))}
                </div>
            ))}
        </div>
    ...
    return (
        <div className="glass-panel rounded-2xl p-6 space-y-6 animate-pulse">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-black/[0.03] rounded-xl space-y-2">
            ...
            <div className="space-y-3">
                <div className="h-3 bg-black/[0.03] rounded w-full skeleton-shimmer" />
                <div className="h-3 bg-black/[0.03] rounded w-3/4 skeleton-shimmer" />
            </div>
...
export function StatSkeleton() {
    return (
        <div className="p-5 rounded-xl bg-black/[0.03] border border-black/[0.06] animate-pulse space-y-2">
            <div className="h-3 bg-white/10 rounded w-20 skeleton-shimmer" />
