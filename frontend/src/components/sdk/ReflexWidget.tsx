...
export const ReflexWidget = ({
    ...
    const containerClasses = cn(
        ...
        isLight ? "bg-white border-zinc-200" : isGlass ? "bg-black/[0.03] backdrop-blur-xl border-black/[0.06]" : "bg-black border-zinc-800",
        isEnabled && !isProtected && "ring-1",
    );

    const textPrimary = isLight ? "text-zinc-950" : "text-[#1A1A1A]";
    ...
    return (
        <div className={containerClasses} style={{ borderColor: isEnabled && !isProtected ? accentColor : undefined }}>
            {/* Header / Brand */}
            {showBranding && (
                <div className={cn("flex items-center justify-between p-3 border-b", isLight ? "border-zinc-100 bg-zinc-50" : "border-black/[0.04] bg-white/[0.02]")}>
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] font-bold uppercase tracking-wider" style={{ color: textSecondary }}>{brandName} Protection</span>
                    </div>
                ...
                <div className="flex items-start justify-between gap-4">
                    <div className="space-y-1">
                        <h3 className={cn("text-xs font-bold flex items-center gap-2", textPrimary)}>
                            ...
                            {surgeData && (
                                <span className="bg-red-500/20 text-red-500 text-[8px] px-1.5 py-0.5 rounded font-bold">SURGE</span>
                            )}
                        </h3>
                        {!compact && (
                ...
                <AnimatePresence>
                        ...
                        >
                            <div className="space-y-3 pt-1">
                                <div className={cn(
                                    ...
                                    surgeData ? "bg-red-500/5 border-red-500/10" : (isLight ? "bg-zinc-50 border-zinc-100" : "bg-black/[0.03] border-black/[0.04]")
                                )}>
                                    <div className="flex items-center justify-between mb-1">
                                        <span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: textSecondary }}>Premium</span>
                                        <span className={cn("text-xs font-bold font-mono", surgeData ? "text-red-500" : textPrimary)}>
                                            {totalPremium.toFixed(2)} USDT
                                        </span>
                                    </div>
                                    {surgeData && (
                                        <div className="flex items-center gap-1.5 text-[8px] text-red-400 font-bold">
                                ...
                                <button 
                                    ...
                                    className={cn(
                                        "w-full py-2.5 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all flex items-center justify-center gap-2",
                                        isLight ? "bg-zinc-900 text-[#1A1A1A]" : "bg-white text-black"
                                    )}
                                >
                                    {isPurchasing ? (
                                        <>
                    ...
                    {isProtected && (
                        ...
                        >
                            ...
                            <div>
                                <h4 className="text-[10px] font-bold text-green-500 uppercase tracking-widest">Protected</h4>
